import type {FfmpegExporterOptions} from '@flammable7841/core';
...
export function getParamDefaultsAndCheckValidity(settings: RenderSettings): {
  ...
  if (
    settings.projectSettings?.exporter?.name === '@flammable7841/core/image-sequence'
  ) {
    throw Error(
  ...
  if (
    settings.projectSettings?.exporter?.name === '@flammable7841/core/wasm' &&
    extension !== 'mp4'
  ) {
    throw Error(
  ...
  if (settings.projectSettings?.exporter?.name === '@flammable7841/core/wasm') {
    return {
      ...defaultReturn,
